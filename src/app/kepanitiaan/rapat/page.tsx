    ...
    return (
      <div className="flex-grow flex items-center justify-center min-h-[50vh]">
        <div className="text-center space-y-4">
          ...
          <p className="text-xs text-slate-400">Memuat Notulen Rapat...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      {showForm && (
        <div className="bg-slate-900/30 border border-slate-800 rounded-2xl p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={resetForm} className="p-2 text-slate-500 hover:text-white rounded-lg">
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input type="date" required value={tanggal} onChange={e => setTanggal(e.target.value)}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-3 py-2.5 text-xs text-white focus:outline-none focus:border-red-500" />
                </div>
              </div>
              <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-400 uppercase">Agenda Utama</label>
              <input type="text" required value={agenda} onChange={e => setAgenda(e.target.value)}
                placeholder="Mis. Rapat Pembentukan Kepanitiaan"
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none focus:border-red-500 font-bold" />
            </div>

            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-400 uppercase">Isi Notulen & Keputusan</label>
              <textarea rows={8} value={notulen} onChange={e => setNotulen(e.target.value)}
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white focus:outline-none focus:border-red-500 leading-relaxed" />
      ...
      <div className="space-y-4">
        {rapatList.map((r) => (
          <div key={r.id} className="bg-slate-900/20 border border-slate-800 rounded-xl overflow-hidden">
            <div className="p-5 flex justify-between items-start md:items-center flex-col md:flex-row gap-4 bg-slate-900/40">
              <div className="space-y-1.5 flex-1">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-[10px] bg-slate-800 text-slate-300 px-2 py-1 rounded-md font-bold flex items-center gap-1">
                ...
                <h3 className="text-base font-black text-white mt-2">{r.agenda}</h3>
              </div>
              
              <div className="flex items-center gap-2 shrink-0">
                <button onClick={() => handleEditClick(r)} className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg transition-colors">
                  <Edit2 className="h-3.5 w-3.5" /> Edit
                </button>
                <button onClick={() => handleDelete(r.id, r.agenda)} className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold bg-slate-800/50 hover:bg-red-500/10 text-slate-400 hover:text-red-400 border border-transparent hover:border-red-500/30 rounded-lg transition-colors">
                  <Trash2 className="h-3.5 w-3.5" /> Hapus
                </button>
              </div>
            </div>
            
            <div className="px-5 pb-5 pt-4 border-t border-slate-800/50">
              <div className="text-xs text-slate-400 uppercase tracking-widest font-black mb-2">Hasil & Notulen Rapat</div>
        ...
        {rapatList.length === 0 && (
          <div className="text-center py-12 border border-slate-800/50 border-dashed rounded-2xl bg-slate-900/10">
            <FileText className="h-8 w-8 text-slate-600 mx-auto mb-3" />
            <p className="text-sm text-slate-400 font-medium">Belum ada rekam notulen rapat.</p>
          </div>
        )}
      </div>
    </div>
